...
function openTxModal(id){
  ...
  document.getElementById('tx-unitprice').value = t.unitPrice || '';
...
['tx-qty','tx-unitprice'].forEach(id=>{
  document.getElementById(id).addEventListener('input', ()=>{
    const qty = Number(document.getElementById('tx-qty').value)||0;
    const price = Number(document.getElementById('tx-unitprice').value)||0;
    if(qty && price) document.getElementById('tx-amount').value = qty*price;
  });
});
...
document.getElementById('save-tx-btn').addEventListener('click', async ()=>{
  ...
  const data = {
    ...
    unitPrice: Number(document.getElementById('tx-unitprice').value)||0,
    amount,
